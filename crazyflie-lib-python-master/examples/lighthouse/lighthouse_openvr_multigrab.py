...
def run_sequence(scf0, scf1):
    ...
    setpoints = [[-1 * pose[2][3], -1 * pose[0][3] - 0.5, pose[1][3] + 0.3], [-1 * pose[2][3], -1 * pose[0][3] + 0.5, pose[1][3] + 0.3]]
    ...
    while True:
        ...
        if not grabbed and trigger:
            ...
            dist0 = vector_norm(vector_subtract(grab_controller_start, setpoints[0]))
            dist1 = vector_norm(vector_subtract(grab_controller_start, setpoints[1]))
        ...
        if trigger:
            ...
            setpoints[closest] = vector_add(grab_setpoint_start, vector_subtract(curr, grab_controller_start))

        cf0.commander.send_position_setpoint(setpoints[0][0], setpoints[0][1], setpoints[0][2], 0)
        cf1.commander.send_position_setpoint(setpoints[1][0], setpoints[1][1], setpoints[1][2], 0)
