...
def run_sequence(scf):
    ...
    while True:
        ...
        for i in range(openvr.k_unControllerStateAxisCount):
            axis_type = vr.getInt32TrackedDeviceProperty(controllerId, openvr.Prop_Axis0Type_Int32 + i)[0]
            if axis_type == openvr.k_eControllerAxis_TrackPad:
                trackpad = (controller_state.rAxis[i].x, controller_state.rAxis[i].y)
        ...
        if trackpad_touched:
            ...
            cf.param.set_value("ring.solidRed", "{}".format(int(rgb[0] * 255)))
            cf.param.set_value("ring.solidGreen", "{}".format(int(rgb[1] * 255)))
            cf.param.set_value("ring.solidBlue", "{}".format(int(rgb[2] * 255)))
        ...
        if trigger:
            ...
            setpoint = vector_add(grab_setpoint_start, vector_substract(curr, grab_controller_start))
