...
@patch("time.sleep")
@patch("cflib.positioning.motion_commander._SetPointThread", return_value=MagicMock(spec=_SetPointThread))
class TestMotionCommander(unittest.TestCase):
    ...
    def test_that_the_estimator_is_reset_on_take_off(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        ...
        # Assert
        self.param_mock.set_value.assert_has_calls([call("kalman.resetEstimation", "1"), call("kalman.resetEstimation", "0")])

    def test_that_take_off_raises_exception_if_not_connected(self, _SetPointThread_mock, sleep_mock):
        # Fixture
    ...
    def test_that_take_off_raises_exception_when_already_flying(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        self.sut.take_off()
    ...
    def test_that_it_goes_up_on_take_off(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        ...
        # Assert
        thread_mock.set_vel_setpoint.assert_has_calls([call(0.0, 0.0, 0.5, 0.0), call(0.0, 0.0, 0.0, 0.0)])
        sleep_mock.assert_called_with(0.4 / 0.5)

    def test_that_it_goes_up_to_default_height(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
        ...
        # Assert
        thread_mock.set_vel_setpoint.assert_has_calls([call(0.0, 0.0, 0.6, 0.0), call(0.0, 0.0, 0.0, 0.0)])
        sleep_mock.assert_called_with(0.4 / 0.6)

    def test_that_the_thread_is_started_on_takeoff(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
    ...
    def test_that_it_goes_down_on_landing(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
    ...
    def test_that_it_takes_off_and_lands_as_context_manager(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
        thread_mock.reset_mock()
        ...
        # Assert
        thread_mock.set_vel_setpoint.assert_has_calls([call(0.0, 0.0, 0.2, 0.0), call(0.0, 0.0, 0.0, 0.0), call(0.0, 0.0, -0.2, 0.0), call(0.0, 0.0, 0.0, 0.0)])
        sleep_mock.assert_called_with(0.3 / 0.2)
        sleep_mock.assert_called_with(0.3 / 0.2)

    def test_that_it_starts_moving_multi_dimensional(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
        self.sut.take_off()
        thread_mock.reset_mock()
    ...
    def test_that_it_starts_moving(self, _SetPointThread_mock, sleep_mock):
        ...
        # Assert
        for line in data:
            self._verify_start_motion(line[0], vel, line[1], _SetPointThread_mock)

    def test_that_it_moves_multi_dimensional_distance(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
        x = 0.1
        y = 0.2
    ...
    def test_that_it_moves(self, _SetPointThread_mock, sleep_mock):
        ...
        # Assert
        for test in data:
            self._verify_move(test[0], vel, test[1], _SetPointThread_mock, sleep_mock)
    ...
    def test_that_it_starts_circle_right(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        velocity = 0.5
        radius = 0.9
    ...
    def test_that_it_starts_circle_left(self, _SetPointThread_mock, sleep_mock):
        # Fixture
        velocity = 0.5
        radius = 0.9
        expected_rate = 360 * velocity / (2 * radius * math.pi)
    ...
    def test_that_it_turns_right(self, _SetPointThread_mock, sleep_mock):
        ...
        # Assert
        thread_mock.set_vel_setpoint.assert_has_calls([call(0.0, 0.0, 0.0, rate), call(0.0, 0.0, 0.0, 0.0)])
    ...
    def test_that_it_circles_right(self, _SetPointThread_mock, sleep_mock):
        ...
        # Assert
        thread_mock.set_vel_setpoint.assert_has_calls([call(velocity, 0.0, 0.0, rate), call(0.0, 0.0, 0.0, 0.0)])
        sleep_mock.assert_called_with(turn_time)
    ...
    def _verify_start_motion(self, function_under_test, velocity, expected, _SetPointThread_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
        thread_mock.reset_mock()
        ...
        try:
            ...
            self._eprint("Failed when testing function " + function_under_test.__name__)
            raise e

    def _verify_move(self, function_under_test, velocity, expected, _SetPointThread_mock, sleep_mock):
        # Fixture
        thread_mock = _SetPointThread_mock()
        ...
        try:
            ...
            print("Failed when testing function " + function_under_test.__name__)
            raise e
...
class TestSetpointThread(unittest.TestCase):
    ...
    def test_that_x_y_and_yaw_is_set(self):
        ...
        self.commander_mock.send_hover_setpoint.assert_called_once_with(x, y, yaw, 0)
