...
KEY_MAPPING = {"up": np.array([1, 0, 0, 0.99]), "down": np.array([-1, 0, 0, 0.99]), "left": np.array([0, 1, 0, 0.99]), "right": np.array([0, -1, 0, 0.99])}  # Up  # Down  # Yaw left  # Yaw right
...
def simulate_sensors(drone_id):
    ...
    ray_results = p.rayTestBatch([pos] * len(ray_directions), [[pos[0] + d[0], pos[1] + d[1], pos[2] + d[2]] for d in ray_directions])
    distances = [r[2] for r in ray_results]  # Trefferpositionen auswerten

    return {"flow_x": optical_flow_x, "flow_y": optical_flow_y, "ranger": distances}
...
def run(
    ...
    logger = Logger(logging_freq_hz=control_freq_hz, num_drones=1, output_folder=output_folder, colab=colab)
    ...
    for i in range(0, NUM_WP):  # for each control step
        ...
        p.resetDebugVisualizerCamera(cameraDistance=2, cameraYaw=-90, cameraPitch=-30, cameraTargetPosition=drone_pos, physicsClientId=env.CLIENT)
