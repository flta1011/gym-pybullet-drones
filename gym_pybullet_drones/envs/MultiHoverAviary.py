...
class MultiHoverAviary(BaseRLAviary):
    ...
    def _computeTruncated(self):
        ...
        for i in range(self.NUM_DRONES):
            if (
                abs(states[i][0]) > 2.0 or abs(states[i][1]) > 2.0 or states[i][2] > 2.0 or abs(states[i][7]) > 0.4 or abs(states[i][8]) > 0.4
            ):  # Truncate when a drones is too far away  # Truncate when a drone is too tilted
                return True
