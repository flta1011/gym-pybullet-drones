...
class BetaAviary(BaseAviary):
    ...
    def __init__(
        ...
        for i in range(self.NUM_DRONES):
            self.sock.append(socket.socket(socket.AF_INET, socket.SOCK_DGRAM))  # Internet  # UDP
            self.sock_pwm.append(socket.socket(socket.AF_INET, socket.SOCK_DGRAM))  # Internet  # UDP
    ...
    def _actionSpace(self):
        ...
        act_lower_bound = np.array([[0.0, 0.0, 0.0, 0.0] for i in range(self.NUM_DRONES)])
    ...
    def _observationSpace(self):
        ...
        obs_lower_bound = np.array(
            [[-np.inf, -np.inf, 0.0, -1.0, -1.0, -1.0, -1.0, -np.pi, -np.pi, -np.pi, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, 0.0, 0.0, 0.0, 0.0] for i in range(self.NUM_DRONES)]
        )
        obs_upper_bound = np.array(
            [
                [np.inf, np.inf, np.inf, 1.0, 1.0, 1.0, 1.0, np.pi, np.pi, np.pi, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, self.MAX_RPM, self.MAX_RPM, self.MAX_RPM, self.MAX_RPM]
                for i in range(self.NUM_DRONES)
            ]
        )
    ...
    def _preprocessAction(self, action):
        ...
        """
        remapped_input = np.array([[action[i][2], action[i][1], action[i][3], action[i][0]] for i in range(self.NUM_DRONES)])  # 0  # 1  # 2  # 3  # Betaflight SITL motor mapping
