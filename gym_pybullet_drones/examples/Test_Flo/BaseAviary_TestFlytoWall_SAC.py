...
class BaseAviary_TestFlytoWall_SAC(gym.Env):
    ...
    def step(self,
        ...
        # print(action)
            ...
            # Convert continuous action to movement direction
        if action[0] > 0.3:  # Forward
            movement = np.array([[1, 0, 0, 0.99]])
        elif action[0] < -0.3:  # Backward
            movement = np.array([[-1, 0, 0, 0.99]])
        else:  # Stay still
            movement = np.array([[0, 0, 0, 0.99]])
        
        # Replace the old discrete action processing
        ...
        # Use the new continuous movement
        action = movement
